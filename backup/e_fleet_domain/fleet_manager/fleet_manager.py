...
# Direct DB connection function that actually works
def direct_db_update(query, params):
    """Direct DB update with explicit commit"""
    try:
        sys.path.append(os.path.dirname(os.path.dirname(__file__)))
        import config
...
class FleetManager(Node):
    def __init__(self):
        ...
        # Using HANABOT_8 and HANABOT_9 for actual fleet operation
        self.robot_name_to_id = {"robot_1": 8, "robot_2": 9}  # Maps to HANABOT_8 and HANABOT_9
        
        # Initialize robots as offline (ERROR) so first message triggers DB update
        self.robots["robot_1"] = RobotState(
            robot_id="robot_1",
            status=RobotStatus.ERROR
        )
        self.robots["robot_2"] = RobotState(
            robot_id="robot_2",
            status=RobotStatus.ERROR
        )
        ...
        for robot_id in self.robots:
            ...
            # Don't force offline - let robots update their own status when they connect
        
        # Status monitoring timer (reduced frequency)
        self.create_timer(5.0, self.monitor_fleet)  # Every 5 seconds instead of 1
        
        # Task assignment timer (check for pending tasks every 5 seconds)
        self.create_timer(5.0, self.check_and_assign_tasks)  # Every 5 seconds instead of 2
        ...
        # Recovery: Reset stuck 'assigned' tasks on startup
    ...
    def recover_stuck_assignments(self):
        ...
        self.destroy_timer(self.recovery_timer)  # Remove this timer after first run
    ...
    def check_and_assign_tasks(self):
        ...
        try:
            ...
            if next_task['task_type'] in ['배달', '호출']:
                ...
                # If coordinates not found, fail the task instead of sending invalid command
                if target_coords is None:
                    self.get_logger().error(f"No coordinates found for resident {next_task['requester_resident_id']}, failing task {next_task['task_id']}")
                    # Reset task to pending instead of failing completely
                    update_task_status(next_task['task_id'], 'pending')
                    # Reset robot state
                ...
                # Verify robot publisher exists
                if robot_id not in self.robot_cmd_pubs:
                    self.get_logger().error(f"No command publisher for {robot_id}, failing task {next_task['task_id']}")
                    update_task_status(next_task['task_id'], 'pending')
                    self.robots[robot_id].status = RobotStatus.IDLE
                    self.robots[robot_id].current_task_id = None
                    return
                ...
                self.get_logger().info(f"Successfully sent command to {robot_id} for task {next_task['task_id']}")
    ...
    def handle_robot_status(self, msg: String, robot_id: str):
        ...
        # Store previous status text for comparison
        previous_status_text = getattr(robot_state, 'last_status_text', None)
        
        # Only process if status actually changed
        if previous_status_text == status_text:
            robot_state.last_update = time.time()
            return  # Skip processing - same status
        
        # Debounce rapid status changes (wait at least 0.5 seconds)
        current_time = time.time()
        last_status_change = getattr(robot_state, 'last_status_change_time', 0)
        if current_time - last_status_change < 0.5:
            # Too rapid - just update timestamp and return
            robot_state.last_update = current_time
            return
        
        # Log only when status changes (and passes debounce)
        if status_text != "idle" or previous_status_text != "idle":
            self.get_logger().info(f"Robot {robot_id} status: {previous_status_text} -> {status_text}")
        
        # Store current status text and change time
        robot_state.last_status_text = status_text
        robot_state.last_status_change_time = current_time
        
        # Check if this is the first connection (robot was offline)
        was_offline = previous_status == RobotStatus.ERROR
        if was_offline:
            self.get_logger().info(f"Robot {robot_id} reconnected!")
        
        # Update robot status based on message
        if status_text == "idle":
            # Only update Fleet Manager status, don't touch DB
            if robot_state.status != RobotStatus.IDLE:
                robot_state.status = RobotStatus.IDLE
                self.get_logger().info(f"Robot {robot_id} -> IDLE")
            ...
            if robot_state.status != RobotStatus.BUSY:
                robot_state.status = RobotStatus.BUSY
                self.get_logger().info(f"Robot {robot_id} -> BUSY ({status_text})")
            
            # Update task status in database to 이동중 (only if not already)
            if robot_state.current_task_id:
                try:
                    with query_db() as cur:
                        # Check current task status first
                        cur.execute("SELECT status FROM tasks WHERE task_id = %s", (int(robot_state.current_task_id),))
                        current_status = cur.fetchone()
                        
                        if current_status and current_status[0] != '이동중':
                            cur.execute("""
                                UPDATE tasks 
                                SET status = '이동중'
                                WHERE task_id = %s
            ...
            if robot_state.status != RobotStatus.BUSY:
                robot_state.status = RobotStatus.BUSY
                self.get_logger().info(f"Robot {robot_id} -> BUSY (picking)")
            
            # Update task status in database to 집기중 (only if not already)
            if robot_state.current_task_id:
                try:
                    with query_db() as cur:
                        # Check current task status first
                        ...
                        if current_status and current_status[0] != '집기중':
                            cur.execute("""
                                UPDATE tasks 
                                SET status = '집기중'
                                WHERE task_id = %s
            ...
            if robot_state.status != RobotStatus.BUSY:
                robot_state.status = RobotStatus.BUSY
                self.get_logger().info(f"Robot {robot_id} -> BUSY (waiting_confirm)")
            
            # Robot arrived at user location - update DB to 수령대기 (only if not already)
            if robot_state.current_task_id:
                try:
                    with query_db() as cur:
                        # Check current task status first
                        ...
                        if current_status and current_status[0] != '수령대기':
                            cur.execute("""
                                UPDATE tasks 
                                SET status = '수령대기'
                                WHERE task_id = %s
            ...
            if robot_state.status != RobotStatus.BUSY:
                robot_state.status = RobotStatus.BUSY
                self.get_logger().info(f"Robot {robot_id} -> BUSY (returning_to_dock)")
    ...
    def update_robot_db_status(self, robot_id: str, status: str):
        """Update robot status in database"""
        bot_id = self.robot_name_to_id.get(robot_id)
        if bot_id:
            success = direct_db_update(
                "UPDATE hana_bots SET status = %s WHERE hana_bot_id = %s",
                (status, bot_id)
            )
            if success and status in ['대기중', '오프라인']:
                self.get_logger().info(f"DB: {robot_id} -> {status}")
            elif not success:
                self.get_logger().error(f"Failed to update DB for {robot_id}")
        ...
        for robot_id, robot_state in self.robots.items():
            if current_time - robot_state.last_update > 5.0:  # 5 seconds timeout
                if robot_state.status != RobotStatus.ERROR:
                    ...
                    # Only broadcast offline status to web - no DB update
                    self.broadcast_robot_status(robot_id, "offline", robot_state.current_task_id)
                ...
                robot_state.status = RobotStatus.IDLE
                # Only broadcast online status to web - no DB update
                self.broadcast_robot_status(robot_id, "idle", robot_state.current_task_id)
