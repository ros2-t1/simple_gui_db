    ...
    try:
        ...
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=DictCursor) as cur:
                ...
                # Get active robots count from DB (not offline)
                cur.execute("""
                    SELECT COUNT(*) as active_robots_today
                    FROM hana_bots 
                    WHERE hana_bot_id IN (8, 9) AND status IS NOT NULL AND status != '오프라인'
                """)
                active_robots = cur.fetchone()['active_robots_today']
                system_metrics['active_robots_today'] = active_robots
...
def admin_robots_api():
    """API endpoint for robot data from database"""
    try:
        robots = []
        
        # Get robots from DB (only HANABOT_8 and HANABOT_9)
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=DictCursor) as cur:
                cur.execute("""
                    SELECT hana_bot_id, bot_name, battery, status
                    FROM hana_bots
                    WHERE hana_bot_id IN (8, 9)  -- Only HANABOT_8 and HANABOT_9
                    ORDER BY hana_bot_id ASC
                """)
                db_robots = cur.fetchall()

        for db_robot in db_robots:
            bot_id = db_robot['hana_bot_id']
            ...
            robots.append({
                ...
                'battery': db_robot['battery'] or 0,
                'status': db_robot['status'] or '오프라인',
                'current_task': current_task,
                'last_update': None  # DB doesn't track last_update
            })
