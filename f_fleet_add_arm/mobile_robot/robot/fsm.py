...
from .arm import arm_pick, arm_place_on_robot, init_arm_communication
...
class DeliveryFSM(Node):
    def __init__(self, robot_name: str, wp_nav: WaypointNavigator):
        ...
        self.current_item = None  # 현재 픽업할 아이템
    ...
    def on_cmd(self, msg: String):
        try:
            ...
            item_name = cmd_data.get("item_name")  # 주문한 아이템 이름
            ...
            if command == "order":
                ...
                if item_name:
                    ...
                    self.get_logger().info(f"주문 아이템: {item_name}")
                elif resident_id:
    ...
    def set_step(self, nxt: int):
        ...
        match self.step:
            ...
            case Step.PICK:
                ...
                # 실제 주문한 아이템으로 픽업
                item_to_pick = self.current_item if self.current_item else "default_item"
                pick_success = arm_pick(item_to_pick)
                if pick_success:
                    arm_place_on_robot()
                    self.get_logger().info(f"로봇암 픽업 완료: {item_to_pick} - 사용자 위치로 이동 시작")
                    self.set_step(Step.GO_TO_USER)
                else:
                    self.get_logger().error(f"로봇암 픽업 실패: {item_to_pick} - IDLE로 복귀")
                    self.set_step(Step.IDLE)
